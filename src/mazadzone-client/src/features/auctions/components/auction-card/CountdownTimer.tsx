 ...
 * 
 */
export function CountdownTimer({ startDate, endDate, variant = "default", className, label, status }: CountdownTimerProps) {
  ...
  const isUrgent = remainingSeconds < 300 && status !== "Upcoming"; // Urgent only for active
    ...
    <div className={cn("flex flex-col items-start gap-1 w-full", className)}>
      <div className="flex w-full items-center justify-between">
        {startDate ? (
          ...
          <div /> // Empty div to push end date to the right if justify-between is used
        )}
        <div className="flex items-center gap-1 text-[10px] font-medium text-muted-foreground uppercase tracking-wider whitespace-nowrap">
